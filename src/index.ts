...
let filesOldMd = await manager.getFilesInDirectory('data//prev data/');
filesOldMd = filesOldMd.filter((file) => file.endsWith('md'));
console.log(filesOldMd);

const fileHander = new FileHandler();
filesOldMd.forEach(async (file) => {
  await fileHander.delete(file);
});
